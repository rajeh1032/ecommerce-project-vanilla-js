    ...
    return `
        <div class="order-card">
            <div class="order-header">
                ...
                <div class="order-status status-${statusRaw}">${statusShow}</div>
            </div>
            ...
            <div class="order-footer">
                ...
                <div class="order-actions">
                    <button class="btn-details" onclick="openModal('${order.id}')">View Details</button>
                    ${statusRaw === 'completed' ? `<button class="btn-reorder">Reorder</button>` : ''}
                </div>
            </div>
        </div>`;
}
...
function filterOrders(status) {
    ...
    if (status === 'all') {
        ...
        displayOrders(allOrders.filter(o => (o.status || '').toLowerCase() === status));
    }
}
...
window.openModal = function (orderId) {
    ...
    document.getElementById('modalBody').innerHTML = `
        <div class="modal-section">
            ...
            <div class="modal-row"><span class="modal-label">Customer</span><span class="modal-value">${escapeHtml(order.customerName)}</span></div>
            <div class="modal-row"><span class="modal-label">Phone</span><span class="modal-value">${escapeHtml(order.phoneNumber)}</span></div>
        </div>

        <div class="modal-section">
            <div class="modal-section-title">Item</div>
            <div class="order-item">
                <div class="item-image"><div class="placeholder-image">📦</div></div>
                <div class="item-details">
                    <div class="item-name">${escapeHtml(order.details)}</div>
                    <div class="item-meta">Qty: ${order.quantity} × $${(parseFloat(order.price) || 0).toFixed(2)}</div>
                </div>
